...
class PaymentIntentCaptureJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function __invoke(PaymentIntentRepositoryInterface $repository): void
    {
        ...
        $nuvei = $paymentIntent->getGatewayPaymentIntent();

        assert($nuvei->getPaymentIntent()->getId()->toString() === (string)$this->account->id);

        $paymentIntent->getGatewayPaymentIntent()
            ->capture(fn(PaymentIntent $nuvei) => PaymentIntent::fromOriginal(
