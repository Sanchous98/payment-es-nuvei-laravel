...
class PaymentIntentCaptureJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function __invoke(PaymentIntentRepositoryInterface $repository): void
    {
        ...
        assert($paymentIntent->getGatewayPaymentIntent()->getPaymentIntent()->getId()->toString() === (string)$this->account->id);

        $paymentIntent
            ->getGatewayPaymentIntent()
            ->capture(function(PaymentIntent $nuvei) use($paymentIntent, $service, $transactionDetailsService) {
                $result = $service->settleTransaction([
                    'clientUniqueId' => $this->message->aggregateRootId()->toString(),
                    'amount' => $this->event->amount ?? $paymentIntent->getMoney()->getAmount(),
